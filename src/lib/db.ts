...
export function suggestWorkJournalContextsFromCVs(
  cvs: Pick<CVSummary, "profile" | "type">[]
...
export async function listHiddenWorkJournalSuggestionKeys(
  supabase: SupabaseClient,
  userId: string
): Promise<Set<string>> {
  const { data, error } = await supabase
  ...
  return new Set(
    (data ?? []).map((item) =>
      contextKey(item.type as WorkJournalContextType, item.name_key as string)
    )
  );
}

export async function listWorkJournalContextSuggestions(
  supabase: SupabaseClient,
  userId: string
): Promise<WorkJournalContextSuggestion[]> {
  const [cvs, contexts, hidden] = await Promise.all([
    listCVs(supabase, userId),
    listWorkJournalContexts(supabase, userId),
    listHiddenWorkJournalSuggestionKeys(supabase, userId),
  ]);
  const existing = new Set(
    contexts.map((context) => contextKey(context.type, context.name))
  );
...
export async function ensureDefaultWorkJournalContext(
  supabase: SupabaseClient,
  userId: string
  ...
  const suggestions = await listWorkJournalContextSuggestions(supabase, userId);
  const best = suggestions[0];
  if (!best) return null;

  return createWorkJournalContext(supabase, {
    user_id: userId,
    type: best.type,
    name: best.name,
...
export async function createWorkJournalContext(
  supabase: SupabaseClient,
  data: {
    user_id: string;
    type: WorkJournalContextType;
...
export async function getWorkJournalContext(
  supabase: SupabaseClient,
  id: string,
  userId: string
): Promise<WorkJournalContext | null> {
...
export async function updateWorkJournalContext(
  supabase: SupabaseClient,
  id: string,
  userId: string,
  data: Partial<Pick<WorkJournalContext, "name" | "role_or_label" | "status" | "is_default">>
): Promise<WorkJournalContext | null> {
  if (data.is_default) {
    await supabase
      .from("work_journal_contexts")
...
export async function hideWorkJournalContextSuggestion(
  supabase: SupabaseClient,
  userId: string,
  input: { type: WorkJournalContextType; name: string }
): Promise<void> {
...
export interface ListWorkJournalEntriesFilters {
...
export async function listWorkJournalEntries(
  supabase: SupabaseClient,
  userId: string,
  filters: ListWorkJournalEntriesFilters = {}
...
export async function getWorkJournalEntry(
  supabase: SupabaseClient,
  id: string,
  userId: string
...
export async function createWorkJournalEntry(
  supabase: SupabaseClient,
  data: Omit<WorkJournalEntry, "id" | "created_at" | "updated_at" | "context">
): Promise<WorkJournalEntry> {
...
export async function deleteWorkJournalEntry(
  supabase: SupabaseClient,
  id: string,
  userId: string
): Promise<boolean> {
