  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <input type="text" value={basics.name ?? ""} onChange={(e) => set("name", e.target.value)} placeholder="Tu nombre completo" className={inputClass} />
      </div>
      <div>
        <label className={labelClass}>Titular</label>
        <input type="text" value={basics.headline ?? ""} onChange={(e) => set("headline", e.target.value)} placeholder="Ej: Senior Software Engineer" className={inputClass} />
      </div>
      ...
      <div>
        ...
        <input type="text" value={basics.location ?? ""} onChange={(e) => set("location", e.target.value)} placeholder="Madrid, España" className={inputClass} />
      </div>
      <div>
        <label className={labelClass}>Enlaces</label>
        <div className="space-y-2 mt-1">
          {(basics.links ?? []).map((link, i) => (
            <div key={i} className="group flex items-center gap-2">
              <input type="text" value={link.label ?? ""} onChange={(e) => updateLink(i, "label", e.target.value)} placeholder="Label" className={`${inputClass} w-1/3`} />
