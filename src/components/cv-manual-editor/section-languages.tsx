    ...
    <div className="space-y-2">
      {items.map((lang, i) => (
        <div key={i} className="group flex items-center gap-2">
          <input type="text" value={lang.name ?? ""} onChange={(e) => update(i, { ...lang, name: e.target.value })} placeholder="Idioma" className={`${inputClass} flex-1`} />
          <input type="text" value={lang.level ?? ""} onChange={(e) => update(i, { ...lang, level: e.target.value })} placeholder="Nivel" className={`${inputClass} w-1/3`} />
